...
export function SUV(){
    ...
    const fetchData =  async () => {
      ...
      try {
        if (selectedOption.value === "Moc rosnąco") {
          ...
          setCars(response.data.filter(el => el.category === 'SUV'));
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const response = await CarService.getCarsSUV();
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const fil = response.data.filter(car => car.category === 'SUV')
        setCars(fil)
        setFilteredData(fil)
        _DATA = fil
      } catch (error) {

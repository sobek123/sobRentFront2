    ...
    const fetchData =  async () => {
      ...
      try {
        if (selectedOption.value === "Moc rosnąco") {
          ...
          setCars(response.data.filter(el => el.category === 'SPORTOWE'));
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const fil = response.data.filter(car => car.category === 'SPORTOWE')
        setCars(fil)
        setFilteredData(fil)
        _DATA = fil
      } catch (error) {
    ...
    return <div style={{marginTop:80}}>
      ...
      />
            ...
            <div>{ cars.length == 0 ? <span><h4 style={{textAlign :'center', marginTop: 200}} id="or">Brak wyników odpowiadających wybranym kryteriom</h4></span> : 
       ...
       return (
        ...
        <Card id={"card"+index} style={{ maxWidth: 400 ,borderRadius:15 }} key={car.id}>
          ...
          <CardContent>
            ...
            <div id="buttons">
            <CardActions>
            {/* { user != null && !car.taken  ? <Button id={"rentButton"+car.id}  style = {{marginLeft:'auto', marginRight: 'auto',color:'blue'}} onClick={() => rent(car.prize, car, index)}><FaCartPlus size={21} style={{marginTop:-4}}></FaCartPlus><span style={{marginLeft: 3}}>Dodaj</span></Button>  : ''} */}
            </CardActions>
            </div>
        </Card>
       )}

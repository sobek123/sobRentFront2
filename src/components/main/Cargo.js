...
export function Cargo(){
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const response = await CarService.getModelsByBrand(brand, 'DOSTAWCZE');
    ...
    const fetchData =  async () => {
      ...
      try {
        if (selectedOption.value === "Moc rosnąco") {
          ...
          setCars(response.data.filter(el => el.category === 'DOSTAWCZE'));
          _DATA = response.data.filter(el => el.category === 'DOSTAWCZE')
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const response = await CarService.getCarsCargo();
    ...
    const fetchData = async () => {
      ...
      try {
        if (ob.brand != '' && ob.transmission == '' && ob.petrol == '' && ob.year == '' && ob.category == '' && ob.numberOfSeats == 0) {
          ...
          const fil = response.data.filter(el => el.category === 'DOSTAWCZE')
          if(ob.model !== ''){
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const fil = response.data.filter(car => car.category === 'DOSTAWCZE')
        setCars(fil)
        setFilteredData(fil)
        _DATA = fil
        // setShowHook(showTab)
    ...
    return <div style={{marginTop:80}}>
      <hr></hr>
        <h4 style={{ width:'100%', textAlign:'center'}}>Grupa Dostawcze</h4>
        <hr></hr>

    ...
    const fetchData =  async () => {
      ...
      try {
        if (selectedOption.value === "Moc rosnąco") {
          ...
          setCars(response.data.filter(el => el.category === 'EKONOMICZNE'));
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const fil = response.data.filter(car => car.category === 'EKONOMICZNE')
        setCars(fil)
        setFilteredData(fil)
        _DATA = fil
      } catch (error) {
    ...
    return <div style={{marginTop:80}}>
      <hr></hr>
        <h4 style={{ width:'100%', textAlign:'center'}}>Grupa Ekonomiczne</h4>
        <hr></hr>
